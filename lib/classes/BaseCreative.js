...
export default class {
  ...
  _setupCurtain() {
    ...
    this.window.document.addEventListener('visibilitychange', event => {
      ...
      if(!curtainUp && event.target.visibilityState == 'visible' && this._allDataReceived()) {
        this.curtainUp();
        curtainUp = true;
      }
    });
  ...
  curtainUp() {
    this.curtain.classList.add('curtain-up');
  }
   ...
   */
  _allDataReceived() {
    return _size(this._dataReceived) >= _size(this._dataEvents);
  }
