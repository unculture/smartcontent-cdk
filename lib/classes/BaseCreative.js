...
export default class {
  ...
  receiveData(event, data) {
    ...
    this._firePreStartIfDataComplete();
  }

  /**
   * Fire preStart if all data has been received.
   */
  _firePreStartIfDataComplete() {
    if (_size(this._dataReceived) < _size(this._dataEvents)) {
      return;
    }

    this._call('preStart', null, true);
    this._checkAssets();
  }
  ...
  /**
   * Available data assets. To be overriden by Creative.
   */
  dataAssets() {
    this._definitionWarning('dataAssets');
    return [];
  }
